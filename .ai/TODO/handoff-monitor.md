...
| Branch | `refactor/monitor` (PR 0 `22cfd38`, PR 1a `2a66eae`, PR 1b `8a0b9f0`, PR 1c `3fb276b`, PR 2 `a9717bb`, PR 3 ASCII `8e45d37`, PR 3 **this commit**). |
| Cadence | **One PR per thread.** This file is the series map. Next is **PR 4** (Histogram + PSD + Spectrogram). **ASCII first + wait** for each view. |
...
Read the spec first (`Key Decisions`, `File layout`, `PR Plan`, Histogram / PSD / Spectrogram tables). This file is implementer order, current-code pitfalls, and the **PR 4** start. Do not re-design graphs, naming, or the lease. Do not change Bands or Raw EEG.
...
4. Reply with the **next-thread prompt in a fenced block** the user can copy into a new chat. Nothing else required from them. Graph PRs (4) keep ASCII-first + wait in that prompt.
...
**PR 4 (this thread). ASCII first + wait for each of Histogram, PSD, Spectrogram. Do not implement painters until each wireframe is approved.**

```
Implement monitor PR 4 only: Histogram + PSD + Spectrogram. ASCII first — paste a GraphShell wireframe for each view (Histogram, PSD, Spectrogram) and wait for approval before painters. Do not implement painters until each ASCII is approved.
Spec: .ai/monitor.md (frozen, rev 5). Handoff: .ai/TODO/handoff-monitor.md (section “This thread — PR 4”).
Histogram is a new AppView. Keep AppView.spectrogram and sidebar Spectrogram. PSD sidebar label PSD. Same electrode toggles as Bands (average membership, default all on, last-one stays). Dart FFT in monitor/dsp.dart: Hamming 256-pt 1/N²; band edges 1–4 / 4–8 / 8–13 / 13–30 / 30–50. Histogram own ±100 µV domain, 64 bins, 8 s default. PSD Welch 4 s default (2/4/8). Spectrogram 20 s default (10/20/30), 0–60 Hz heatmap. Reuse GraphShell; Record hidden until 5a. Do not change Bands or Raw EEG.
...
| **3** | **this commit** — Bands on GraphShell. One `TimeSeriesPane`, five series, mean of selected in dB. Electrode toggles (all on, last-one stays). Default 30 s; pinch-X → `custom`. Overshoot hold isolated in `overshoot_hold.dart`. Old `views/bands.dart` + `chart_controller.dart` deleted. Record hidden. |

### PR 3 shipped

- `monitor/views/bands_view.dart` + `panes/time_series_pane.dart` + `electrode_toggles.dart` + `panes/overshoot_hold.dart`
- GraphShell `custom` dropdown when `windowSeconds` ∉ `windowOptions`
- `ViewportController` strip helpers (`followStrip` / `pinchX` / `bandsWindowOptions`) without breaking EEG
- Mean in dB (`10·log10` per electrode, then mean). BandCache stays linear.
- `.ai/ui-map.md` Bands chrome
...
| **4** | Histogram + PSD + Spectrogram (`AppView.histogram` only) | **yes** (each view) — **this thread** | 2; ∥ 3 |
...
| GraphShell | `monitor/graph_shell.dart` | Shared chrome. Record slot `showRecord: false`. When `windowSeconds` ∉ `windowOptions`, closed label is **`custom`**. |
| Bands | `monitor/views/bands_view.dart` | One `TimeSeriesPane`, five series, dB Y, electrode toggles, 30 s default, pinch-X `custom`. |
...
| Histogram / PSD / Spectrogram | `views/psd_view.dart`, `views/terminal.dart` | **Placeholders.** PR 4 painters after ASCII. |
...
**ASCII first.** Paste a GraphShell wireframe for **each** of Histogram, PSD, and Spectrogram. **Wait** for approval before that view’s painter. Do not implement all three painters after one ASCII. Spec tables are locked (axes, windows, bins, FFT); the wireframe is chrome + layout.

When ASCII is approved and painters are done, follow **Close the thread** (commit + rewrite this file for PR 5a + fenced next-thread prompt). Do not skip it.
...
- **Histogram** — new `AppView.histogram`. Sidebar **Histogram**. One pane. Amplitude histogram of the visible window. X: µV, **own** domain (default **±100 µV**, overflow ±50 / ±200), independent of Raw EEG `SharedYScale`. Y: **count**. Default window **8 s**. Fixed **64** linear bins. Mean of selected electrodes.
- **PSD** — keep `AppView.psd`. Sidebar label **PSD** (shorten from `Power Spectral Density (PSD)`). One pane, **no overlay**. Welch of last T seconds, mean of selected. X: Hz **0–60** (0–100 overflow). Y: **log power**. Default **4 s** (discrete 2 / 4 / 8). 1 s Hamming segments, 50% hop, **256-point** FFT. Band shading: delta **1–4**, theta **4–8**, alpha **8–13**, beta **13–30**, gamma **30–50**. Peak marker: argmax in 8–13 Hz, one label.
- **Spectrogram** — keep `AppView.spectrogram` and sidebar **Spectrogram**. Widget `monitor/views/spectrogram_view.dart`. Heatmap: X = time, Y = **0–60 Hz**, color = log power. Same STFT as PSD of the **mean** of selected. Default **20 s** (10 / 20 / 30). Hop ~0.25 s. “Waterfall” may appear **once** in docs, **not** on screen.
- FFT: **`monitor/dsp.dart` only.** No FFT package in `pubspec.yaml`. Local Cooley–Tukey, Hamming, power `(re²+im²)/(n*n)`, `n = 256`.
- EEG RAM is `SweepBuffer` only. Histogram/PSD/Spectrogram `getRange` through a thin adapter over `SweepBuffer.sampleAt` (index / 256 Hz). Do **not** add a second LiveCache EEG ring.
...
- Implement painters before ASCII approval for that view.
- Record button (5a).
- Change Bands or Raw EEG. Do not put chips on Raw EEG. Do not add `custom` to EEG window options. Do not reuse Bands overshoot hold.
- Rename Spectrogram → Waterfall. No `AppView.waterfall`. No `AppView.recordings`.
- FFT package. New FFI. Linear-Y default on PSD. Log bins / KDE / per-channel overlay on Histogram. Stacked per-channel spectrograms.
...
```
lib/src/monitor/
  dsp.dart                         # NEW — Cooley–Tukey Hamming 256-pt 1/N²
  panes/histogram_pane.dart        # NEW
  panes/psd_pane.dart              # NEW
  panes/spectrogram_pane.dart      # NEW
  views/histogram_view.dart        # NEW
  views/psd_view.dart              # NEW
  views/spectrogram_view.dart      # NEW
  monitor.dart                     # export
  viewport_controller.dart         # histogram/psd/spectrogram window options if needed

lib/src/settings.dart              # AppView.histogram; _viewFromName / _viewNames
lib/src/app.dart                   # sidebar + body widgets
...
- DSP: Hamming 256-pt, power `1/N²`, band edges 1–4 / 4–8 / 8–13 / 13–30 / 30–50
...
Do **not** run FRB. Do **not** `cargo check --target aarch64-linux-android`. Visual graphs cannot be verified in CI — human `flutter run` after ASCII approval.

### PR 4 done when

- ASCII approved then painters for Histogram, PSD, Spectrogram
...
- **PR 4 is ASCII first.** Paste three wireframes. Wait. Then painters.
...
- **FFT matches Rust:** Hamming, `1/N²`, 256-pt, edges 1–4 / 4–8 / 8–13 / 13–30 / 30–50. Do not invent edges.
- **Histogram Y-scale is own ±100 µV**, not Raw EEG `SharedYScale`.
