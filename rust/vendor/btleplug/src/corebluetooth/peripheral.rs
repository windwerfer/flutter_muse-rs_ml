...
impl Peripheral {
    ...
    pub(crate) fn new(
        ...
        let (notifications_channel, _) = broadcast::channel(256);
            ...
            loop {
                match event_receiver.next().await {
                    ...
                    Some(PeripheralEventInternal::Disconnected) => {
                        info!(
                            "Peripheral {} disconnected, exiting event loop.",
                            shared.uuid
                        );
                        // Don't emit DeviceDisconnected here — it is already
                        // emitted by the adapter when it processes the
                        // CoreBluetoothEvent::DeviceDisconnected event.
                        // Just break out of the loop so the task exits cleanly.
                        break;
                    }
                    None => {
    ...
    async fn disconnect(&self) -> Result<()> {
        ...
        match fut.await {
            CoreBluetoothReply::Ok => {
                // DeviceDisconnected is already emitted by the adapter when it
                // processes the CoreBluetooth didDisconnectPeripheral callback,
                // so we don't emit it again here.
                trace!("Device disconnected!");
            }
            CoreBluetoothReply::Err(msg) => return Err(Error::RuntimeError(msg)),
            _ => error!("Shouldn't get anything but Ok or Err!"),
        }
        Ok(())
    }
