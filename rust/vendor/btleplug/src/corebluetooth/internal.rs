...
impl PeripheralInternal {
    ...
    fn check_discovered(&mut self) {
        ...
        if !self.services.values().any(|service| !service.discovered) {
            if self.connected_future_state.is_none() {
                // This can happen if a disconnect was processed while
                // service/characteristic discovery was still in flight.
                // confirm_disconnect already errored the future, so just bail.
                warn!("All services discovered but no connected future to fulfil — likely disconnected mid-discovery");
                return;
            }
            let services = self
                .services
    ...
    pub fn confirm_disconnect(&mut self) {
        ...
        // If a connection was in progress, error that future too.
        ...
        // Fulfill all pending characteristic and descriptor futures
        ...
        self.services.iter().for_each(|(_, service)| {
            service
                ...
                .for_each(|(_, characteristic)| {
                    ...
                    // Also error any pending descriptor futures
                    for (_, descriptor) in descriptors.iter() {
                        for state in descriptor.read_future_state.iter() {
                            state.lock().unwrap().set_reply(error.clone());
                        }
                        for state in descriptor.write_future_state.iter() {
                            state.lock().unwrap().set_reply(error.clone());
                        }
                    }
                });
...
impl CoreBluetoothInternal {
    ...
    fn on_peripheral_connection_failed(
        ...
        if let Some(peripheral) = self.peripherals.get_mut(&peripheral_uuid) {
            if let Some(future) = peripheral.connected_future_state.take() {
                ...
                error!(
                    "Connection failed for {} but no future was waiting: {}",
                    peripheral_uuid, error
                );
            }
        }
    }
    ...
    fn on_characteristic_written(
            ...
            self.get_characteristic(peripheral_uuid, service_uuid, characteristic_uuid)
        {
            ...
            if let Some(state) = characteristic.write_future_state.pop_back() {
                ...
                warn!("Got characteristic written event but no future was waiting");
            }
        }
    }
    ...
    fn connect_peripheral(&mut self, peripheral_uuid: Uuid, fut: CoreBluetoothReplyStateShared) {
        ...
        if let Some(p) = self.peripherals.get_mut(&peripheral_uuid) {
            ...
            error!("Peripheral {} not found for connect", peripheral_uuid);
            fut.lock()
                .unwrap()
                .set_reply(CoreBluetoothReply::Err(String::from("Device not found")));
        }
    }
    ...
    fn disconnect_peripheral(&mut self, peripheral_uuid: Uuid, fut: CoreBluetoothReplyStateShared) {
        ...
        if let Some(p) = self.peripherals.get_mut(&peripheral_uuid) {
            ...
            // Peripheral already gone (e.g. unexpected disconnect already
            // processed).  Fulfil the future so callers don't hang.
            trace!("Peripheral {} already removed, fulfilling disconnect future", peripheral_uuid);
    ...
    fn is_connected(&mut self, peripheral_uuid: Uuid, fut: CoreBluetoothReplyStateShared) {
        if let Some(p) = self.peripherals.get_mut(&peripheral_uuid) {
            ...
            // Peripheral removed from the map → disconnected.
            fut.lock()
                .unwrap()
    ...
    fn write_value(
        ...
        fut.lock()
            .unwrap()
            .set_reply(CoreBluetoothReply::Err(String::from("Device disconnected")));
    }
                    ...
                    if let Some(descriptor) = characteristic.descriptors.get_mut(&descriptor_uuid) {
                        ...
                        if let Some(state) = descriptor.read_future_state.pop_back() {
                            state
                                .lock()
                                .unwrap()
                                .set_reply(CoreBluetoothReply::ReadResult(data.clone()));
                        } else {
                            warn!("Got descriptor read event but no future was waiting");
                        }
                    }
                }
            }
    ...
    fn on_descriptor_written(
        ...
        if let Some(descriptor) = self.get_descriptor(
            ...
            if let Some(state) = descriptor.write_future_state.pop_back() {
                ...
                warn!("Got descriptor written event but no future was waiting");
            }
        }
    }
            ...
            delegate_msg = self.delegate_receiver.select_next_some() => {
                match delegate_msg {
                    ...
                    CentralDelegateEvent::DidUpdateState{state} => {
                        // When the adapter powers off, proactively disconnect all
                        // peripherals so that disconnect events propagate properly
                        // even if CoreBluetooth does not call didDisconnectPeripheral
                        // for every connected peripheral.
                        if state == CBManagerState::PoweredOff {
                            let peripheral_uuids: Vec<Uuid> =
                                self.peripherals.keys().cloned().collect();
                            for uuid in peripheral_uuids {
                                self.on_peripheral_disconnect(uuid).await;
                            }
                        }
...
mod tests {
    ...
    /// Generate a unique UUID for each test call (no `v4` feature needed).
    fn test_uuid() -> Uuid {
    ...
    // ---------------------------------------------------------------
    // BtlePlugFuture tests
    // ---------------------------------------------------------------

    #[test]
    fn future_resolves_when_reply_set_before_poll() {
        ...
        assert!(matches!(result, CoreBluetoothReply::Ok));
    }

    #[test]
    fn future_resolves_when_reply_set_after_poll() {
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        // Spawn a task that will set the reply after a short delay.
    ...
    fn future_resolves_with_error() {
        ...
        state
            .lock()
            .unwrap()
            .set_reply(CoreBluetoothReply::Err("test error".into()));

        let result = futures::executor::block_on(fut);
        match result {
            CoreBluetoothReply::Err(msg) => assert_eq!(msg, "test error"),
    ...
    fn future_panics_on_double_set() {
        let state: CoreBluetoothReplyStateShared =
    ...
    // ---------------------------------------------------------------
    // CoreBluetoothInternal message dispatch tests
    //
    // These test that sending messages for unknown peripherals properly
    // fulfils (errors) the futures instead of silently dropping them.
    // We construct a CoreBluetoothInternal via its channel interface.
    // ---------------------------------------------------------------

    /// Helper: build a CoreBluetoothInternal and its channels.
    /// Only possible on macOS because CBCentralManager requires ObjC runtime.
    #[cfg(target_vendor = "apple")]
    fn make_cbi() -> (
        CoreBluetoothInternal,
        Sender<CoreBluetoothMessage>,
        Receiver<CoreBluetoothEvent>,
    ) {
        let (msg_tx, msg_rx) = mpsc::channel::<CoreBluetoothMessage>(256);
        let (evt_tx, evt_rx) = mpsc::channel::<CoreBluetoothEvent>(256);
        let cbi = CoreBluetoothInternal::new(msg_rx, evt_tx);
        (cbi, msg_tx, evt_rx)
    }

    #[cfg(target_vendor = "apple")]
    #[test]
    fn connect_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        let uuid = test_uuid();
        cbi.connect_peripheral(uuid, state);

        let result = futures::executor::block_on(fut);
        match result {
            CoreBluetoothReply::Err(msg) => assert!(msg.contains("not found")),
            other => panic!("Expected Err, got {:?}", other),
        }
    }
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn disconnect_unknown_peripheral_returns_ok() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        let uuid = test_uuid();
        cbi.disconnect_peripheral(uuid, state);
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn is_connected_unknown_peripheral_returns_disconnected() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        let uuid = test_uuid();
        cbi.is_connected(uuid, state);

        let result = futures::executor::block_on(fut);
        match result {
            CoreBluetoothReply::State(s) => {
                assert_eq!(s, CBPeripheralState::Disconnected);
            }
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn read_value_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        cbi.read_value(test_uuid(), test_uuid(), test_uuid(), state);

        let result = futures::executor::block_on(fut);
        assert!(matches!(result, CoreBluetoothReply::Err(_)));
    }

    #[cfg(target_vendor = "apple")]
    #[test]
    fn write_value_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn subscribe_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        cbi.subscribe(test_uuid(), test_uuid(), test_uuid(), state);
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn unsubscribe_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        cbi.unsubscribe(test_uuid(), test_uuid(), test_uuid(), state);
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn read_descriptor_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        cbi.read_descriptor_value(
            test_uuid(),
            test_uuid(),
            test_uuid(),
            test_uuid(),
    ...
    #[cfg(target_vendor = "apple")]
    #[test]
    fn write_descriptor_unknown_peripheral_errors_future() {
        let (mut cbi, _, _) = make_cbi();
        let fut = CoreBluetoothReplyFuture::default();
        let state = fut.get_state_clone();

        cbi.write_descriptor_value(
            test_uuid(),
            test_uuid(),
            test_uuid(),
            test_uuid(),
    ...
    // ---------------------------------------------------------------
    // on_peripheral_disconnect for unknown peripheral is a no-op
    // ---------------------------------------------------------------
    #[cfg(target_vendor = "apple")]
    #[tokio::test]
    async fn disconnect_unknown_peripheral_is_noop() {
        let (mut cbi, _, mut evt_rx) = make_cbi();
        let uuid = test_uuid();

        // Should not panic, should not send any event.
        cbi.on_peripheral_disconnect(uuid).await;

        // Give a moment for any event to propagate
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;

        // No DeviceDisconnected event should have been dispatched
