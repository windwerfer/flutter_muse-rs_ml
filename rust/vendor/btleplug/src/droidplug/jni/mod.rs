...
/// Returns a JNIEnv for the current thread, attaching the thread to the JVM
/// first if it isn't already attached. `JavaVM::get_env` alone fails with
/// `JNI_EDETACHED` on threads that aren't yet attached to the JVM (e.g.
/// tokio worker threads). This wrapper ensures the calling thread is always
/// attached so that JNI calls from async Rust code don't fail.
