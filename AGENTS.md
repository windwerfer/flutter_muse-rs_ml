...
- **Notes edits rewrite the whole container crash-safely**: `SessionStore.updateNotes` (`session_store.dart`) re-reads the full `.muse.feedback`, swaps `metadata.notes`, re-encodes, and writes via `SessionStorage.writeFileAtomic` (`session_storage.dart`). Filesystem = `.name.tmp` sibling + `rename()` (atomic on POSIX); SAF = native `writeFileAtomic` (`MainActivity.kt`) writes `name.mtmp`, deletes old, `renameDocument`, and `recoverDoc()` heals an interrupted swap on the next read (`.mtmp` entries are skipped in `listFiles`). The dashboard (`feedback_dashboard.dart`) shows a corner save chevron only when notes are dirty, a spinner while saving, and a brief check flash after; a `PopScope` intercepts back with an "Unsaved notes" Save/Stay/Discard dialog in the read-only history view.
