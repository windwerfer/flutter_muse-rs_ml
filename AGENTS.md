...
- **Signal gate + autodrop model** (`lib/src/feedback/`): the "how good the headband fit is" concept is a *list of 4 per-pad quality scores* (`AppUiState.signalQuality`, 0–100, from EEG std), separate from the *bands data* actually used to compute ATR. Intention (reused by future feedback options):
